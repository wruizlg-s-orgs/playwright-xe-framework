...
test.describe('Profile Authorization API', () => {
    test('@smoke Should access profile with valid token', async ({
        authenticatedApi,
    }) => {
        ...
        SchemaAssertions.validate(body, ProfileSchema);

        expect(body.username).toBe('admin');
    });

    test('@regression Should reject profile access without token', async ({
        api,
    }) => {
